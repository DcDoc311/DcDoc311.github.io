...
1990년대 초반, Windows가 막 보급되던 시기에 개발자들은 고민에 빠졌습니다. "사용자가 Windows용 프로그램을 DOS 환경에서 실행하면 어떻게 될까?"

그 해결책이 바로 DOS Stub입니다.
...
Optional Header라는 명칭은 COFF(Common Object File Format) 명세에서 목적 파일(`.obj`)에는 선택 항목이었던 것이 PE 포맷으로 계승되면서 그대로 굳어진 것입니다. 실행 파일에서는 반드시 존재해야 하며, Windows 로더가 가장 많이 참조하는 구조체입니다.
...
파일의 물리적인 특징을 담고 있습니다. "이 파일이 어떤 설계도로 그려졌는가?"를 알려줍니다.
...
- **왜 고정값을 가지나요?** 컴파일러가 코드를 만들 때, 함수 호출이나 변수 참조 주소를 미리 계산해야 하기 때문입니다. 예를 들어 "0x401000에 있는 함수를 실행해"라는 명령을 미리 적어두려면 기준점(`ImageBase`)이 필요합니다.
    
- **어떻게 프로그램이 메모리에 올라가나요?**
...
#### 3.4.3.1. 0x0 ~ ImageBase 사이에는 무엇이 있나요?

결론부터 말씀드리면, **그 앞부분은 대부분 "접근 금지 구역"이거나 "운영체제 전용 구역"** 입니다.
###### ① NULL 포인터 보호 구역 (0x0 ~ 0xFFFF)

가장 낮은 주소 영역(대략 64KB)은 보통 **Null Page**라고 부르며, 운영체제가 의도적으로 **비워두고 접근을 차단**합니다.

- **이유:** 프로그래머가 실수로 `null` 포인터를 참조했을 때(주소 0에 접근), 즉시 에러를 발생시켜 프로그램이 이상하게 동작하기 전에 강제 종료하기 위해서입니다. 만약 여기에 실제 데이터가 있다면 프로그램이 경고 없이 망가질 수 있습니다.

###### ② 시스템 및 공유 라이브러리 구역

`ImageBase` 이전의 주소들에는 Windows 시스템 운영에 필요한 핵심 데이터나, 모든 프로세스가 공유하는 시스템 DLL(예: `ntdll.dll`) 등이 로드될 공간이 미리 예약되어 있습니다.

---

#### 3.4.3.2. 왜 일정 기준(ImageBase)을 넘겨서 로드하나요?

###### ① 가상 주소 공간의 독립성
우리가 보는 `0x400000`은 실제 RAM의 물리 주소가 아니라 **가상 주소**입니다.

- 모든 프로그램은 "나는 이 컴퓨터의 메모리를 혼자 다 쓰고 있다"라고 착각하도록 설계되어 있습니다.
- `ImageBase`라는 기준점을 정해두면, 운영체제는 여러 프로그램을 동시에 실행하더라도 각각의 가상 메모리 주소를 겹치지 않게 관리하기가 훨씬 수월해집니다.

##### ② 효율적인 메모리 관리 (Paging)
Windows는 메모리를 **페이지(4KB)** 단위로 쪼개서 관리합니다. `ImageBase`가 `0x400000`처럼 큰 숫자로 정해진 이유는 CPU와 운영체제가 메모리를 읽어올 때 계산하기 편한 **정렬된(Aligned)** 위치이기 때문입니다.

---

#### 3.4.3.3. 프로그램이 메모리에 올라가는 과정

프로그램 실행 파일(`.exe`)이 하드디스크에서 메모리로 올라가는 과정은 이렇습니다:

1. **공간 확보:** Windows 로더가 파일을 읽고 `ImageBase`와 `SizeOfImage`를 확인한 뒤, 가상 메모리에 그만한 크기의 메모리를 할당합니다. 
2. **헤더 복사:** 파일의 맨 앞부분(DOS Header, PE Header)을 메모리의 시작 지점에 그대로 복사합니다.
3. **섹션 배치:** 파일에 정의된 각 섹션(`.text`, `.data`)들을 헤더에 적힌 주소에 맞춰서 하나씩 배치합니다. 이때 아까 배운 **Section Alignment** 규칙에 맞춰 빈칸을 띄우며 배치합니다.
4. **재배치(Relocation):** 만약 `0x400000` 주소에 이미 다른 게 들어있다면, 운영체제는 다른 빈 주소를 찾아 프로그램을 올리고, 코드 안의 주소값들을 새 주소에 맞게 일일이 수정해 줍니다
...
- **왜 이렇게 하나요?** 컴퓨터는 데이터를 1바이트씩 읽는 것보다 정해진 규격(배수)에 맞춰 읽는 것이 훨씬 빠르기 때문입니다. 마치 책꽂이에 책을 꽂을 때, 책 크기와 상관없이 일정한 칸막이 간격을 유지하는 것과 같습니다.
### 3.4.5. DataDirectory (128 bytes)

Optional Header의 마지막 부분에는 **Data Directory**라는 배열이 있습니다. 여기에는 Export Table(함수 내보내기), Import Table(외부 DLL 가져오기), Resource Table(아이콘, 이미지) 등 중요한 테이블들의 위치 정보가 담겨 있습니다.
...
- **내용:** "우리 DLL에는 `Add()`, `Sub()`라는 함수가 있고, 그 주소는 여기야!"라고 외부에 알려주는 목록입니다. 다른 프로그램들이 이 DLL의 기능을 빌려 쓸 때 이 테이블을 참조합니다.

#### ② Import Table (가져오기 테이블)

- **역할:** 프로그램이 실행되기 위해 외부에서 빌려오는 함수들의 목록입니다.
- **내용:** "난 `kernel32.dll`의 `CreateFile` 함수가 필요해"라고 적어둡니다. 프로그램이 실행될 때 로더가 이 테이블을 보고 필요한 DLL들을 메모리에 함께 올려줍니다.

#### ③ Resource Table (리소스 테이블)

- **역할:** 코드(로직)가 아닌 **데이터**들을 관리합니다.
- **내용:** 프로그램 아이콘, 메뉴 구성, 대화 상자 디자인, 문자열 테이블, 심지어 내장된 이미지나 소리 데이터가 어디에 있는지 정의합니다. 우리가 C# 프로젝트에서 `Resources.resx`에 넣는 파일들이 결국 이곳에 정리됩니다.
...
- **특징:** 아까 배운 `FileAlignment`의 배수가 되어야 합니다. 실제 데이터가 100바이트라도 정렬 단위가 512바이트라면 이 값은 512가 됩니다.

## 4.5. PointerToRawData (4 bytes)

- **설명:** 파일 내부에서 섹션 데이터가 시작되는 **물리적 위치(Offset)** 입니다.
- **활용:** 우리가 나중에 **섹터 리더**를 만들 때, "파일의 0x400 번지부터 읽어라"라고 명령을 줄 때 사용하는 아주 중요한 값입니다. `FileAlignment`의 배수여야 합니다.

## 4.6. PointerToRelocations (4 bytes)

- **설명:** 해당 섹션의 재배치 정보(Relocation)가 시작되는 파일 오프셋입니다.
- **현재:** 보통 실행 파일(`.exe`)에서는 사용되지 않으며, 주로 목적 파일(`.obj`)에서 사용됩니다. 일반적인 PE 분석에서는 `0`인 경우가 많습니다.
...
- **예:** `.text` 섹션은 보통 "읽기 + 실행" 권한을 가지며, `.data` 섹션은 "읽기 + 쓰기" 권한을 가집니다.
...
결론적으론 임베디드 Bootloader와 큰 차이가 없는 것 같습니다. 아마 다른 운영체제 혹은 다른 시스템에서도 비슷하게 구성되어 있을 것으로 예상됩니다. 

추가적으로 PE 포맷에는 이 글에서 다루지 않은 영역이 존재합니다. Optional Header의 DataDirectory 인덱스 6번은 Debug Directory를 가리키며, 이를 따라가면 PDB 파일의 경로와 GUID가 기록된 RSDS 구조체에 도달합니다. PDB는 PE 파일 외부에 존재하지만 PE 포맷 안에 그 위치가 명시되는 구조입니다. 다만 PDB 내부는 멀티 스트림 기반의 독자적인 포맷으로 구성되어 있어, 별도 글에서 다루는게 적절해 보입니다. 
